...
def epsilon(Y, n, t):
    ...
    for i in range(4*n-t):
        for j in range(4*n-t):
            e = np.abs(Y[i][j])*math.sqrt(4*n-t)-1
            if abs(e) > max:
                max = abs(e)
    return max
...
def read_hadamard(filepath):
    ...
    d = H.shape[0]
    H = (1/np.sqrt(d))*H
    print(f"Loaded Hadamard matrix of shape {H.shape}")
    return H, (d//4)

# Input file path here to use a different Hadamard matrix
H, n = read_hadamard("had.160.tpal.txt")
...
print("e_Hadamard: ", epsilon(H, n, 0))
print("e_Y1: ", epsilon(Y1, n, t))
print("e_Y2: ", epsilon(Y2, n, t))